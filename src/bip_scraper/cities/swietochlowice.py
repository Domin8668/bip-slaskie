...
class SwietochlowiceScraper:
    ...
    def scrape_acts(self, *, now: datetime) -> list[LegalAct]:
        ...
        try:
            ...
            raise RuntimeError(f"Swietochlowice scraper: failed to scrape acts") from exc
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt >= self.max_retries:
                    break
                time.sleep(2)
        raise RuntimeError(f"Swietochlowice scraper: request failed for {url}") from last_error
    ...
    def _parse_acts_from_html(self, page_html: str, *, now: datetime) -> list[LegalAct]:
        ...
        for link in soup.select("a[href]"):
            ...
            results.append(
                LegalAct(
                    stable_id=f"swietochlowice-uchwala-{stable_id}",
                    title=text,
